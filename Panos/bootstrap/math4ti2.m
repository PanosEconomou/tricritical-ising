...
hilbert::usage = "hilbert[mat] computes the Hilbert basis of the monoid generated by the rows of mat over the non-negative integers."
...
hilbert[mat_List] := Module[
    {basename, result},
    basename = StringJoin["4ti2-hilbert-", ToString[$SessionID]];
    
    (* Write as .lat not .mat -- rows are monoid generators *)
    writeMatrix[mat, basename, "lat"];
    
    RunProcess[{hilbertcmd, basename}];
    
    result = {
        readFile[basename, "hil"],
        readFile[basename, "zfree"]
    };
    
    Scan[(deleteFile[basename, #])&, {"lat", "hil", "zfree"}];
    result
];
